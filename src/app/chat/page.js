...
export default function ChatPage() {
  const [messages, setMessages] = useState([
    {
      id: "welcome",
      sender: "bot",
      text: "أهلاً بك في صفحة المساعد الذكي! 🦷 أنا بوت التحليلات والمساعدة الخاص بالورشة. كيف يمكنني خدمتك اليوم؟ يمكنك اختصار الوقت وسؤالي عن إجمالي الأرباح، عدد الأطباء، أو الكونترات قيد الصيانة حالياً.",
    },
  ]);
  ...
  const generateBotResponse = (text) => {
    ...
    if (cleanText.includes("صيانة") || cleanText.includes("معدة") || cleanText.includes("جهاز") || cleanText.includes("هاندبيس") || cleanText.includes("الرف") || cleanText.includes("رف")) {
      return `يوجد حالياً ${stats.activeOrdersCount} أكونتؤات قيد الصيانة على الرف حالياً. يمكنك متابعتها أو تسليمها في صفحة الصيانة. 🛠️`;
    }
    ...
    if (cleanText.includes("مرحبا") || cleanText.includes("أهلاً") || cleanText.includes("اهلاً") || cleanText.includes("سلام") || cleanText.includes("هيلو")) {
      return "أهلاً بك! أنا مساعد الورشة الذكي. كيف يمكنني مساعدتك اليوم؟ يمكنك الاستفسار عن الأرباح، الدكاترة المسجلين، أو كونترات قيد الإصلاح. 🦷🤖";
    }
    ...
    return "عذراً، لم أفهم سؤالك بدقة"
  };
  ...
  const promptSuggestions = [
    ...
    { label: "الكونترات على الرف", desc: "استعلم عن الكونترات التي هي قيد الصيانة حالياً", icon: <Wrench size={16} />, query: "كم عدد الكونترات قيد الصيانة حالياً؟" },
  ];
